...
module.exports.downloadReport = async function (req, res) {
	try {
		...
		if (companies.length === 0) {
			req.flash("error", "Please Add Some Companies First");
			return res.redirect("back");
		}
		//If the Interviews are not found
		if (interviews.length === 0) {
			req.flash("error", "Please Add Some Interview Slots First");
			return res.redirect("back");
		}
		...
		if (results.length === 0) {
			req.flash(
				"error",
				"Please Add Students & Results to the Interview Slots First"
			);
			return res.redirect("back");
		}
